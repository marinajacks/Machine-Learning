# -*- coding: utf-8 -*-
"""
Created on Sat Jan  6 18:58:58 2018
...
import numpy as np
import os
#p='E:\project\Machine-Learning\PCA\digits\\trainingDigits'
def eachFile(filepath):
    pathDir =  os.listdir(filepath)
    abspaths=[]
    for i in range(len(pathDir)):
        path=filepath+'\\'+str(pathDir[i])
        abspaths.append(path)
    flags=[]
    imgs=[]
    for i in range(len(pathDir)):
            flags.append(pathDir[i][0])
            f=open(abspaths[i])
            lists=[]
            for i in ((list(f.read().strip().replace('\n','')))):
                lists.append(int(i))
            imgs.append(lists)
            f.close()
    imgs=np.array(imgs)
...
'''这个是计算的测试的准确度,'''
def accuracy(p,p1):
    p2='E:\project\Machine-Learning\PCA\digits\\testDigits'
    initflag,initdata=eachFile(p2)

    names,meanVals,redEigVects=train(p)
    
    testflag=[]
    for i in range(len(initdata)):
        testflag.append(test(names,meanVals,redEigVects,initdata[i]))
