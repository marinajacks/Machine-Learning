...
'''PCA的公式,主要是为了获取到训练数据的均值,特征向量信息,这就是训练学习过程'''
def pcas(dataMat,percentage=0.8):
    meanVals=np.mean(dataMat,axis=0)  #对每一列求平均值，因为协方差的计算中需要减去均值
    meanRemoved=dataMat-meanVals
    ...
    eigValInd=eigValInd[:-(k+1):-1] #从排好序的特征值，从后往前取k个，这样就实现了特征值的从大到小排列
    redEigVects=eigVects[:,eigValInd]   #返回排序后特征值对应的特征向量redEigVects（主成分）
    lowDDataMat=(meanRemoved+meanVals)*redEigVects #将原始数据投影到主成分上得到新的低维数据lowDDataMat
    reconMat=(lowDDataMat*redEigVects.T)+meanVals   #得到重构数据reconMat
    return lowDDataMat,reconMat

'''这部分是为了进行根据分类的结果进行划分的数据,返回了每一位数据的标签以及数据将降维后的
对应的数据'''
def trains(p):
    datamats=loadiris(p)
    flags=loadflags(p)
    ...
    for i in range(len(datamat)):
        lowDDataMat,reconMat=pcas(datamat[i],percentage=0.8)
        lowDDataMats.append(lowDDataMat)
        reconMats.append(reconMat)
    
    return names,lowDDataMats,reconMats


p='E:\\project\\Machine-Learning\\PCA\\trainiris.txt'
names,lowDDataMats,reconMats=trains(p)   
...
x1=np.array(lowDDataMats[0][:,0])
y1=np.array(lowDDataMats[0][:,1])
plt.scatter(x1,y1,color='r')
x2=np.array(lowDDataMats[1][:,0])
y2=np.array(lowDDataMats[1][:,1])
plt.scatter(x2,y2,color='g')
x3=np.array(lowDDataMats[2][:,0])
y3=np.array(lowDDataMats[2][:,1])
